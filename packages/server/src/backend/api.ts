...
let marketFeed: MarketFeed | null = null;
...
async function sessionTick(config: UserSessionConfig) {
    ...
    if (Object.keys(feedSnapshot).length === 0 && marketFeed) {
        auditService.logWarn('[Market] Market feed snapshot is empty');
    }
...
export function startBackend(config: UserSessionConfig, restoreState?: DashboardState) {
    if (sessionInterval) {
        ...
        void setupMarketFeed(liveConfig);
    ...
    sessionInterval = setInterval(() => sessionTick(config), SYSTEM_TIMINGS.BACKEND_TICK_MS);
}
...
async function setupMarketFeed(config: UserSessionConfig): Promise<void> {
    if (!state) {
        return;
    }
    ...
    if (marketFeed) {
        try {
            ...
            console.warn('[MarketFeed] Failed to stop existing feed', error);
        }
        marketFeed = null;
    }

    if (pairs.length > 0) {
        marketFeed = new MarketFeed({ symbols: pairs, logger: console });
        try {
            await marketFeed.start();
        } catch (error) {
            console.error('[MarketFeed] Failed to start market feed', error);
        }
    }
}

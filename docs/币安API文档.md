# 甯佸畨 API 鏂囨。锛堝畬鏁存嫹璐濓級

浠ヤ笅涓?BINANCE.txt 鐨勫畬鏁村唴瀹规寜鍘熸枃鍛堢幇锛圲TF-8锛夛紝渚涘紑鍙戞绱娇鐢ㄣ€俙r
...
400 鈥?your request failed, see error for the reason.
403 鈥?you have been blocked by the Web Application Firewall.
409 鈥?your request partially failed but also partially succeeded, see error for details.
418 鈥?you have been auto-banned for repeated violation of rate limits.
429 鈥?you have exceeded API request rate limit, please slow down.
...
session.logon 鈥?authenticate, or change the API key associated with the connection
session.status 鈥?check connection status and the current API key
session.logout 鈥?forget the API key associated with the connection
...
An aggregate trade (aggtrade) represents one or more individual trades. Trades that fill at the same time, from the same taker order, with the same price 鈥?those trades are collected into an aggregate trade with total quantity of the individual trades.
...
TIF	Description
GTC	Good 'til Canceled 鈥?the order will remain on the book until you cancel it, or the order is completely filled.
IOC	Immediate or Cancel 鈥?the order will be filled for as much as possible, the unfilled quantity immediately expires.
FOK	Fill or Kill 鈥?the order will expire unless it cannot be immediately filled for the entire quantity.
Notes:
...
STOP_ON_FAILURE 鈥?if cancellation request fails, new order placement will not be attempted.
ALLOW_FAILURE 鈥?new order placement will be attempted even if the cancel request fails.
Request	Response
cancelReplaceMode	orderRateLimitExceededMode	Unfilled Order Count	cancelResult	newOrderResult	status
STOP_ON_FAILURE	DO_NOTHING	Within Limits	鉁?SUCCESS	鉁?SUCCESS	200
鉂?FAILURE	鉃?NOT_ATTEMPTED	400
鉁?SUCCESS	鉂?FAILURE	409
Exceeds Limits	鉁?SUCCESS	鉁?SUCCESS	N/A
鉂?FAILURE	鉃?NOT_ATTEMPTED	N/A
鉁?SUCCESS	鉂?FAILURE	N/A
CANCEL_ONLY	Within Limits	鉁?SUCCESS	鉁?SUCCESS	200
鉂?FAILURE	鉃?NOT_ATTEMPTED	400
鉁?SUCCESS	鉂?FAILURE	409
Exceeds Limits	鉂?FAILURE	鉃?NOT_ATTEMPTED	429
鉁?SUCCESS	鉂?FAILURE	429
ALLOW_FAILURE	DO_NOTHING	Within Limits	鉁?SUCCESS	鉁?SUCCESS	200
鉂?FAILURE	鉂?FAILURE	400
鉂?FAILURE	鉁?SUCCESS	409
鉁?SUCCESS	鉂?FAILURE	409
Exceeds Limits	鉁?SUCCESS	鉁?SUCCESS	N/A
鉂?FAILURE	鉂?FAILURE	N/A
鉂?FAILURE	鉁?SUCCESS	N/A
鉁?SUCCESS	鉂?FAILURE	N/A
CANCEL_ONLY	Within Limits	鉁?SUCCESS	鉁?SUCCESS	200
鉂?FAILURE	鉂?FAILURE	400
鉂?FAILURE	鉁?SUCCESS	409
鉁?SUCCESS	鉂?FAILURE	409
Exceeds Limits	鉁?SUCCESS	鉁?SUCCESS	200
鉂?FAILURE	鉂?FAILURE	400
鉂?FAILURE	鉁?SUCCESS	N/A
鉁?SUCCESS	鉂?FAILURE	409
Notes:
...
Query information about all your orders 鈥?active, canceled, filled 鈥?filtered by time range.
...
All active subscriptions for a session can be found using session.subscriptions.
Limits
...
The client should send a Logout <5> message to close the session before disconnecting. Failure to send the logout message will result in the session鈥檚 SenderCompID (49) being unusable for new session establishment for a duration of 2x the HeartInt (108) interval.
...
If the client has not received any messages within a HeartInt (108) interval, the client is responsible for sending a TestRequest <1> to ensure the connection is healthy. Upon receiving such a TestRequest <1>, the server will respond with a Heartbeat <0> containing the expected TestReqID (112). If the client does not receive the server鈥檚 response within a HeartInt (108) interval, the client should close the session and connection and establish new ones.
API Key Permissions
...
ExecutionReport<8> with ExecType (150) value NEW (0) if the order was accepted.
ExecutionReport<8> with ExecType (150) value REJECTED (8) if the order was rejected.
...
ExecutionReport<8> with ExecType (150) value CANCELED (4) for each canceled order.
ListStatus<N> if orders in an order list were canceled.
OrderCancelReject<9> if cancellation was rejected.
Reject<3> if the message is rejected.
...
ExecutionReport<8> with ExecType (150) value CANCELED (4) for the canceled order.
ExecutionReport<8> with ExecType (150) value NEW (0) for the new order.
ExecutionReport<8> with ExecType (150) value REJECTED (8) if the new order was rejected.
OrderCancelReject<9> if the cancellation was rejected.
Reject<3> if the message is rejected.
...
ExecutionReport<8> with ExecType (150) value CANCELED (4) for the every order canceled.
OrderMassCancelReport<r> with MassCancelResponse (531) field indicating whether the message is accepted or rejected.
Reject<3> if the message is rejected.
...
=>562	MinTradeVol	QTY	N	Corresponds to the LOT_SIZE filter
=>1140	MaxTradeVol	QTY	N	Corresponds to the LOT_SIZE filter
=>25039	MinQtyIncrement	QTY	N	Corresponds to the LOT_SIZE filter
=>25040	MarketMinTradeVol	QTY	N	Corresponds to the MARKET_LOT_SIZE filter
=>25041	MarketMaxTradeVol	QTY	N	Corresponds to the MARKET_LOT_SIZE filter
=>25042	MarketMinQtyIncrement	QTY	N	Corresponds to the MARKET_LOT_SIZE filter
=>969	MinPriceIncrement	PRICE	N	Corresponds to the PRICE filter
=>2551	StartPriceRange	PRICE	N	Corresponds to the PRICE filter
=>2552	EndPriceRange	PRICE	N	Corresponds to the PRICE filter
Sample message:
...
2 - SELL
=>25043	FirstBookUpdateID	INT	N	Only present in Diff. Depth Stream.
...
=>25044	LastBookUpdateID	INT	N	Only present in Diff. Depth Stream and Individual Symbol Book Ticker Stream.
...
market_orders_faq	Detailed information on the behavior of Market Orders
...
order_amend_keep_priority	Detailed Information on the behavior of Order Amend Keep Priority
pegged_orders	Detailed Information on Pegged Orders
sbe_faq	Information on the implementation of Simple Binary Encoding (SBE) on the API
testnet	Information on SPOT Testnet
Change log
